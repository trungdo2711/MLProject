...
\lstinputlisting[language=Python, caption={training.py --- Full training pipeline (data loading, RF, XGB \& LGB training, ensemble weight search)}, label={lst:training}]{src/training.py}
...
After all three models are trained independently, the \texttt{find\_best\_weights()} function performs a 3-way grid search over $w_{\text{RF}}, w_{\text{XGB}}, w_{\text{LGB}} \in \{0.0, 0.1, \ldots, 1.0\}$ subject to the constraint $w_{\text{RF}} + w_{\text{XGB}} + w_{\text{LGB}} = 1.0$, to find the weighting that maximizes the \textbf{F\textsubscript{2} score} on the held-out test set.
...
\textbf{Observed Result.}\quad In the current deployment, the grid search converged to \texttt{w\_rf = 0.0}, \texttt{w\_xgb = 1.0}, and \texttt{w\_lgb = 0.0}, meaning the system relies \textit{exclusively} on XGBoost for the probability estimation, with both Random Forest and LightGBM receiving zero weight. This outcome is consistent with XGBoost's superior F\textsubscript{2} score on the held-out test set. All three models remain loaded and available for ensemble rebalancing should the training data distribution change in future iterations.
...
The F\textsubscript{2}-optimized weight search converged to \texttt{w\_rf = 0.0}, \texttt{w\_xgb = 1.0}, and \texttt{w\_lgb = 0.0}, meaning both Random Forest and LightGBM contribute \textit{nothing} to the final probability despite being loaded and maintained. This represents:
\begin{itemize}
    \item \textbf{Reduced model diversity:} A true ensemble derives its strength from combining diverse learners. With only one model active out of three, the system loses the variance-reduction benefit and becomes fully dependent on XGBoost's specific learned decision boundaries.
    ...
    \item \textbf{Wasted computational resources:} The Random Forest model occupies $\approx 414$\,MB on disk and LightGBM adds further overhead; both are loaded into memory at startup, consuming resources without contributing to inference.
\end{itemize}
